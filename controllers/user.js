...
const getRecipe = async (req, res, next) => {
  try {
    const imageUrl =
      "https://food-recog.onrender.com/images/" + req.savedImages[0];
      ...
      .then(async function (apiResponse) {
        ...
        predictions.forEach((item) => {
          ...
          if (
            ...
            item.confidence > classGroups[className].confidence
          ) {
            classGroups[className] = {
              class: className,
        ...
        const sonuc = allRecipes
          .map((recipe) => {
            ...
            const kullanilmayanIngredients = gelenIngredients
              .filter((item) => !ingredients.includes(item.class))
              .map((item) => item.class);
          ...
          .sort(
            (a, b) =>
              b.kullanilanIngredients.length - a.kullanilanIngredients.length
          ); // kullanılan malzemelerin sayısına göre sırala

        res.json({ sonuc, algilananIngredients: gelenIngredients });
      })
      .catch(function (error) {
        console.log("ROBOFLOW HATASI:", error.message);
